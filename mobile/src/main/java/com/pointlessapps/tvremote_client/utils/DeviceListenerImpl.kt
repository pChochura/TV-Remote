...
class DeviceListenerImpl(private val deviceWrapper: DeviceWrapper) : Device.Listener() {
	...
	override fun onShowIme(device: Device, editorInfo: EditorInfo, p2: Boolean, extractedText: ExtractedText?) {
		deviceWrapper.onShowIme?.invoke(device, editorInfo, extractedText)
	}
}